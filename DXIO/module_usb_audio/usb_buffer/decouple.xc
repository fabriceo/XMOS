    ...
    while(1)
    {
...
#endif
        {
            ...
            if (tmp == SET_SAMPLE_FREQ)
            {
                ...
                printf("SET_SAMPLE_FREQ sampFreq %d\n",sampFreq);
#endif
...
#if defined(PRINTF) && (defined XSCOPE)
                printf("SET_STREAM_FORMAT_IN numchan %d, subslot %d, format %d\n",g_numUsbChan_In,g_curSubSlot_In,dataFormat);
#endif
...
#if defined(PRINTF) && (defined XSCOPE)
                printf("SET_STREAM_FORMAT_OUT numchan %d, subslot %d, format %d, sampRes %d\n",g_numUsbChan_Out,g_curSubSlot_Out,dataFormat,sampRes);
#endif
                /* Reset OUT buffer state */
                SET_SHARED_GLOBAL(g_aud_from_host_rdptr, aud_from_host_fifo_start);
                SET_SHARED_GLOBAL(g_aud_from_host_wrptr, aud_from_host_fifo_start);
