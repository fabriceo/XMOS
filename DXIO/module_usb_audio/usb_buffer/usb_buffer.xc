...
// time stamp each usb packet and sof
int g_usb_in_heartBeat = 0;
int g_usb_out_heartBeat = 0;
int g_SOF_heartBeat = 0;
            ...
            )
{
    ...
    while(1)
    {
        ...
        select
        {
            ...
            int time = getTime() - g_usb_in_heartBeat;
            g_usb_in_heartBeat += time;
            if (time>10000000)
                printf("usb in  restarted >100ms\n");
            else
                if (time>20000) {
                    unsigned l;
                    {time, l} = mac(time, 42949673, 0, 0);  // = 2^32 / 100
                    printf("usb in delayed by %dus\n",time);
                }
#endif
...
#if defined(PRINTF) && defined(XSCOPE)
            int time = getTime() - g_usb_out_heartBeat;
            g_usb_out_heartBeat += time;
            if (time>10000000)
                printf("usb out restarted >100ms\n");
            else
                if (time>20000) {
                    unsigned l;
                    {time, l} = mac(time, 42949673, 0, 0);  // = 2^32 / 100
                    printf("usb out delayed by %dus\n", time);
                }
#endif
