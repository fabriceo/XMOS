...
void Endpoint0(chanend c_ep0_out, chanend c_ep0_in, chanend c_audioControl,
    ...
    unsigned dfuInterface) // CLIENT_INTERFACE(i_dfu, dfuInterface)
{
    ...
    while(1)
    {
        ...
        XUD_Result_t result = USB_GetSetupPacket(ep0_out, ep0_in, &sp);
...
#if defined(PRINTF) && defined (XSCOPE ) && defined( PRINTF_USBREQ )
            char * requestName[14] = {
            "GET_STATUS",
            "CLEAR_FEATURE",
            "REQ2?",
            ...
            "other" };
            char * dir[2] = { "H>D", "D>H" };
            char * typ[] = { "STD","CLASS","VEND","?TYP" };
            char * recip[] = { "DEV", "INT", "EP", "OTHER"};
            int requestIdx = sp.bRequest;
            if (requestIdx > 0xC) requestIdx = 0xD;
            int requestRecip = sp.bmRequestType.Recipient;
            if (requestRecip>3) requestRecip = 3;
#endif
        if (result == XUD_RES_OKAY)
        {
#if defined(PRINTF) && defined (XSCOPE ) && defined( PRINTF_USBREQ )
            if ((sp.bRequest != USB_CLEAR_FEATURE)
            &&  (sp.bRequest != USB_GET_DESCRIPTOR))
                printf("?USB [%2X] %s wValue=0x%3X, wIndex=0x%4X, %s, length=%d, %s, %s\n",
            sp.bRequest, requestName[requestIdx], sp.wValue,   sp.wIndex,   dir[sp.bmRequestType.Direction], sp.wLength, typ[sp.bmRequestType.Type],recip[sp.bmRequestType.Recipient]);
#endif
            result = XUD_RES_ERR;
...
#if defined(PRINTF) && defined (XSCOPE ) && defined( PRINTF_USBREQ )
        if ((result == XUD_RES_OKAY) && (resultPrev != XUD_RES_OKAY))
            ;//printf("?USB treated by usb_standard\n");
        else
        if (resultPrev == XUD_RES_OKAY) ;//printf("?USB treated by specific\n");
        else
            printf("?USB [%2X] %s wValue=0x%3X, wIndex=0x%4X, %s, length=%d, %s, %s not treated ???\n",
                        sp.bRequest, requestName[requestIdx], sp.wValue,   sp.wIndex,   dir[sp.bmRequestType.Direction], sp.wLength, typ[sp.bmRequestType.Type],recip[sp.bmRequestType.Recipient]);
#endif
